...
\begin{abstract}
  ...
  % A bigram (or digram) is a group of two successive letters or words.
  % Frequency analysis of bigrams is commonly used in cryptanalysis or machine learning.
  % This paper presents a concurrent program written in C++ that counts the frequency of bigrams
  % in a text. The concurrent program is compared to the sequential counterpart.
  % The last section of the paper shows the performance difference between the two programs.
  % The main tools used are the std::Thread class and the Boost Library.
\end{abstract}
...
Nella relazione si descrive un programma parallelo scritto in C++ che conta le frequenze dei bigrammi di lettere in un testo.
Nella sezione [bla] si mostrano inoltre le differenze prestazionali tra il programma parallelo e la sua controparte sequenziale, mostrando
...
% A bigram is a sequence of two adjacent letters or words. Bigrams are used for a variety of different task.
% Statistical analysis of text with bigrams is commonly used in cryptanalysis, machine learning, speech recognition and so on.
% This paper presents a program written in C++ that counts the frequency of bigrams of letters in a text using the standard Thread class
% and the Boost Library. The main program is then compared to the sequential counterpart.
% Execution times with various text files and Speedup are shown to mark the performance difference between the two programs.
\vspace{3cm}
...
usata è la ricerca del bigramma trovato e il consecutivo incremento del contatore.
...
In Algorithm~\ref{alg:algBase} vi è descritto l'argoritmo in speudocodice.
% To get the bigrams frequency count, the program must parse every single word and save how many
% occurrences of the letters it finds. The data structure used to count the occurences is the Hashtable.
% This data structure was chosen because the most used operation in the program
% is finding the bigram and increasing the number of its counter. In an Hashtable
% the search operation has $O(1)$ time complexity (on average) and it's suitable for the purpose.
\begin{algorithm}
